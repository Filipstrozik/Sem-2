...
.data
	...
	WYBOR:               	.asciiz "\n0) Wydrukuj wynik \n1) Jako notacje postfiksowa \nWybierz numer opcji:\n"	

	BLAD_WPROWADZENIA_OPCJI:.asciiz "Blad wprowadzenia opcji\n"
                        	.align 2
	BLAD_TEKSTU:     	.asciiz "Nie dalo sie przerobic tekstu\n"
				.align 2
	PRZETWARZANIE:		.asciiz "Przetwarzanie...\n"
				.align  2				
...
l0_begin_print_postfix:
  	move $a0, $s0
  	jal size_deque

  	beq $v0, $0, l0_end_print_postfix
  	...
  	lb $t0, ($t0)
  	lb $t1, ($v0)
  	bne $t0, $t1, is_op #nie jest numerkiem czyli skacze
  	lb $a0, 1($v0)
  	addi $a0, $a0, 48		#48 = '0'
...
l0_continue_print_postfix:
	...
	j l0_begin_print_postfix	
	
l0_end_print_postfix:
	li $v0, 11
	li $a0, 0
	syscall
...
return_token_p:
	sub $a1, $a0, 48	# 48 = '0' nie wiem czy to potrzebne
...
token_is_operator:
	...
	lb $t0, ($s3)		#operator ze stosu
	

 	lb $t1, ($s2)		#aktualny operator z kolejki
 	
	
	beq $t1, 7, ominzamiane
...
l0_begin_rpn_parser:
	...
	lb $t1, ($v0)
  	...
  	#przetlumacz na float i wrzuc na stos
